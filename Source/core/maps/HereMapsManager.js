var HereMapsManager = function(footprintCore, settingsProvider) {
    ...
    script.innerHTML = 'window.here.features.angularDebug=true; function updateRouteMode() {var routes = document.getElementsByClassName("route_card"); for (var i = routes.length - 1; i >= 0; i--) {var mode = angular.element(routes[i]).scope().route.mode.transportModes[0]; routes[i].setAttribute("data-mode",mode); } };';
...
HereMapsManager.prototype.getDistanceString = function (route) {
    var distanceString = route.getElementsByClassName('distance')[0].innerHTML; // Using innerText instead of innerHTML results in strange errors.
...
HereMapsManager.prototype.update = function() {
    ...
    for(var i=0; i < routes.length; i++) {
        ...
        thisMap.insertFootprintElement(routes[i], thisMap.footprintCore.createFootprintElement(distanceInKm));
        if (thisMap.settingsProvider.showTravelCost())
            thisMap.insertTravelCostElement(routes[i], thisMap.footprintCore.createTravelCostElement(distanceInKm));
    }
    },1000);
};
