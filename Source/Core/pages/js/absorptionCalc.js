...
var absorptionForm = {
	...
	submit : function(form) {
		...
		// console.log(data);
		var emission = (data.unitSystem != "metric"? 0.453592 : 1) * data.emission,
		...
		if($('#customTreeCheckbox').is(':checked')) {
			...
			var height = (data.unitSystem == "metric"? 3.28084 : 1) * data.treeHeight,
				diameter = (data.unitSystem == "metric"? 0.393701 : 1) * data.treeDiameter,
				age = data.treeAge;

			var weight = (diameter>=11 ? 0.15:0.25) * height * diameter * diameter;
			 ...
			 */
			treeRate = weight * 1.2 * 0.725 * 0.5 * 3.67 * 0.453592/ age;
		}
		else {
		...
		var timeStr = 'It will take <b>' + form.timeString(treeMins) + '</b>for a <b>' + tree + '</b> tree to absorb <b>' + data.emission + ' ' + unit + '</b> of CO<sub>2</sub>.';

		var respMins = ( emission / 1.04326 ) * 24 * 60;
		var respStr = 'The same amount of CO<sub>2</sub> will be produced by a human (through respiration) in <b>' + form.timeString(respMins) + '</b>.';
		...
		var carStr = 'Also, a car would have to travel <b>' + distance.toFixed(2) + ' ' + distUnit + '</b> to produce <b>' + data.emission + ' ' + unit + '</b> of CO<sub>2</sub>.';
	...
	init : function() {
		...
		$.getJSON(browserServices.getFilePath('/core/resources/trees.json'), function(response) {
	    	...
	    	for (var key in form.treeStats) {
	    		$('#treeName').append('<option value="' + key + '">' + key + '</option>');
	    	}
