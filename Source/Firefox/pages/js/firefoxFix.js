...
function getStorage(key, cb) {
	self.port.emit('storageGetRequest', {tag: storageGetBufferIndex, storageKey: key});
...
function saveStorage(data) {
	self.port.emit('storageSetRequest', {data: data});
}

var clone = cloneInto(firefoxApi, unsafeWindow);

exportFunction(getTranslation, clone, {defineAs: 'getTranslation'});
exportFunction(getStorage, clone, {defineAs: 'getStorage'});
exportFunction(saveStorage, clone, {defineAs: 'saveStorage'});
